...
- **2026-05-28** — comm-13 fixed and verified: `/api/webhooks/twilio-sms-status` built; `sendViaTwilio` passes StatusCallback; `Activity.description` stores MessageSid for lookup; failed delivery creates Activity + owner Notification. `__tests__/twilio-sms-status-webhook.test.ts` added.
