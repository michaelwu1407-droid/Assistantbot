...
- `fixed` WhatsApp **notifications** feature (per-type opt-in toggles + two-way action-code replies) is app-complete on branch `claude/add-whatsapp-notifications-qRuh5`: new `NotificationChannelPref` table, shared `executeNotificationAction`, notification settings card, dispatch gated by `WHATSAPP_NOTIFICATIONS_ENABLED`, 19 new unit tests green. Feature ships disabled until the same Twilio `63007` sender-registration block is cleared — identical provider blocker as the existing WhatsApp assistant.
