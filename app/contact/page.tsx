      ...
      <main>
        ...
        <section className="pt-32 pb-16 px-6 relative overflow-hidden isolate bg-[linear-gradient(180deg,#F5F7F8_0%,#F4F7F5_60%,#F7F6F3_100%)]">
          ...
          <div className="relative z-10 mx-auto max-w-3xl text-center flex flex-col items-center gap-6">
            ...
            <motion.h1 {...fadeUp(0.04)} className="text-4xl md:text-6xl font-extrabold tracking-[-0.04em] leading-[1.07] text-midnight text-balance">
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <motion.div {...fadeUp(0.12)} className="flex justify-center mb-12">
              <div className="flex items-center gap-2 p-1.5 bg-slate-100 rounded-2xl">
                <button
                  ...
                  className={`py-2.5 px-6 rounded-xl text-sm font-semibold transition-all shadow-sm ${billingPeriod === "monthly" ? "bg-white text-midnight" : "text-slate-500 hover:text-midnight shadow-none bg-transparent"}`}
                >
                  Monthly
                </button>
                <div className="relative">
                  <button
                    ...
                    className={`py-2.5 px-6 rounded-xl text-sm font-semibold transition-all shadow-sm ${billingPeriod === "yearly" ? "bg-white text-midnight" : "text-slate-500 hover:text-midnight shadow-none bg-transparent"}`}
                  >
                    Yearly
                  </button>
            ...
            <motion.div {...fadeUp(0.16)} className="grid md:grid-cols-2 gap-6 items-end">
              ...
              <div className="rounded-[18px] border border-slate-200 bg-slate-50 p-8 opacity-80 flex flex-col items-center text-center">
                ...
                <Button size="lg" variant="outline" className="w-full text-slate-400 border-slate-200 cursor-not-allowed hover:bg-transparent hover:text-slate-400 pointer-events-none">
                  Good luck with that
                </Button>
              </div>
              ...
              <div className="rounded-[18px] border-2 border-primary bg-white shadow-2xl shadow-primary/10 p-8 relative flex flex-col items-center text-center -mt-6">
                ...
                <div className="text-xs text-slate-500 mb-8 max-w-[220px]">
                  {billingPeriod === "monthly" ? "Cancel anytime. No lock-in contracts." : "Stripe promo codes accepted at checkout."}
                </div>
                ...
                <Link href="/auth" className="w-full">
                  <Button size="lg" variant="mint" className="w-full h-14 text-[15px] shadow-xl shadow-primary/20">
        ...
        <section className="py-20 px-6 bg-white border-t border-slate-200/60">
          <div className="max-w-4xl mx-auto">
            <motion.div {...fadeUp()} className="text-center mb-14">
              ...
              <p className="mt-3 text-slate-600 max-w-2xl mx-auto">Stop losing money on missed calls, forgotten follow-ups, and wasted admin hours.</p>
            </motion.div>
            <div className="grid md:grid-cols-3 gap-6">
              {STATS.map((stat, i) => (
                <motion.div key={i} {...fadeUp(0.1 + i * 0.1)} className="bg-slate-50/50 rounded-[18px] p-8 border border-slate-200 shadow-sm text-center flex flex-col items-center justify-center">
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {FEATURES.map((f, i) => {
                const Icon = f.icon
                return (
                  <motion.div key={f.title} {...fadeUp(0.1 + i * 0.05)} className="bg-white border border-slate-100 p-5 rounded-[18px] flex flex-col items-start gap-4 hover:shadow-md transition-shadow">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <Input id="name" name="name" placeholder="Your name" required className="bg-slate-50/50" />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="email">Email</Label>
                          <Input id="email" name="email" type="email" placeholder="you@example.com" required className="bg-slate-50/50" />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <Label htmlFor="phone">Phone (optional)</Label>
                        <Input id="phone" name="phone" type="tel" placeholder="+61 400 000 000" className="bg-slate-50/50" />
                      </div>

                      <div className="space-y-2">
                        <Label htmlFor="subject">Subject</Label>
                        <Input id="subject" name="subject" placeholder="Brief summary of your enquiry" required className="bg-slate-50/50" />
                      </div>
