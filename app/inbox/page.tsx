...
export default async function InboxPage() {
  ...
  try {
    ...
    const [activities, contacts] = await Promise.all([
      getActivities({ workspaceId: workspace.id, typeIn: ["EMAIL", "CALL", "NOTE", "TASK", "MEETING"] }),
  ...
  if (dbError || !workspace || !interactions) {
    return (
      <div className="h-screen flex items-center justify-center">
        <p className="text-muted-foreground">Database not initialized. Please push the schema first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-card">
      <div className="h-16 border-b border-border flex items-center px-4 shrink-0">
        <Link
          href="/crm/dashboard"
          className="mr-4 p-2 rounded-full hover:bg-muted text-muted-foreground transition-colors"
        >
