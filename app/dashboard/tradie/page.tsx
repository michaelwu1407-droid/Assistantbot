...
import { MapPin, Phone, MessageSquare, Wrench, Camera, Navigation, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function TradiePage() {
  ...
  const [jobStatus, setJobStatus] = useState<'PENDING' | 'TRAVELING' | 'ARRIVED' | 'WORKING'>('PENDING');
  const [showSafetyCheck, setShowSafetyCheck] = useState(false);

  const handleMainAction = async () => {
    if (jobStatus === 'PENDING') {
      setJobStatus('TRAVELING');
      await updateJobStatus('mock-job-id', 'TRAVELING');
    } else if (jobStatus === 'TRAVELING') {
      setJobStatus('ARRIVED');
      setShowSafetyCheck(true);
      await updateJobStatus('mock-job-id', 'ARRIVED');
    }
  };
    ...
    <div className="relative h-full w-full bg-slate-950 text-slate-50 overflow-hidden font-sans">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 p-4 flex justify-between items-start pointer-events-none">
        <div className="pointer-events-auto">
          <h1 className="text-lg font-bold">Good Morning, Scott</h1>
        ...
        <div className="px-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <h2 className="text-xl font-bold text-white mt-1">Mrs. Jones - Blocked Drain</h2>
              <p className="text-slate-400 text-sm">8:00 AM • 12 Smith St, Collingwood</p>
            </div>
