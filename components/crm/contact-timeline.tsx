        ...
        <CardContent>
          {deals.length === 0 ? (
            ...
            <div className="space-y-3">
              {deals.map(deal => (
                <div key={deal.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg border border-border/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Stage: {getUserFacingDealStageLabel(deal.stage)}</p>
                    </div>
                  </div>
            ...
            {activities.map((activity) => {
              ...
              return (
                <div key={activity.id} className="relative">
                  <div className="absolute -left-[31px] top-0 h-6 w-6 rounded-full bg-card border border-border flex items-center justify-center">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="app-body-secondary">{activity.description || activity.title}</p>
                  </div>
                </div>
              )
            })}
