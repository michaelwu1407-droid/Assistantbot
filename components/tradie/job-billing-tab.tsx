        ...
        <div className="space-y-2 p-3 bg-card border border-border rounded-lg">
            {items.map((it, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                    ...
                    {items.length > 1 && (
                        <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-red-500" onClick={() => remove(idx)}>
                            <X className="w-3 h-3" />
                        </Button>
                    )}
                </div>
                            ...
                            <Card key={inv.id} className={`overflow-hidden border-border shadow-sm ${inv.status === "VOID" ? "opacity-60" : ""}`}>
                                ...
                                {inv.status !== "VOID" && editingId !== inv.id && (
                                    <div className="bg-muted/30 p-2 flex gap-2 flex-wrap border-t border-border/50">
                                        ...
                                        {(inv.status === "DRAFT" || inv.status === "ISSUED") && (
                                            <Button size="sm" variant="outline" className="bg-card border-red-200 text-red-600 hover:bg-red-50 text-xs h-8" disabled={busy} onClick={() => handleVoid(inv.id)}>
