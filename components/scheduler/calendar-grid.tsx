                ...
                <div className="grid" style={{ ...gridStyle, minWidth }}>
                    ...
                    {visibleDates.map(date => (
                        <div key={`header-${date.toISOString()}`} className={cn(
                            "sticky top-0 z-20 h-[60px] p-2 border-b border-r font-semibold text-sm text-center flex flex-col items-center justify-center bg-slate-50",
                    ...
                    {HOURS.map(hour => (
                        <div key={`row-${hour}`} className="contents">
                            {/* Time Label */}
                            <div className="sticky left-0 z-10 bg-white border-b border-r flex items-center justify-center text-xs text-muted-foreground font-medium p-2 min-h-[100px]">
