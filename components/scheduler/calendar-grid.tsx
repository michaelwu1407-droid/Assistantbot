...
export function CalendarGrid({ scheduledJobs, visibleDates }: CalendarGridProps) {
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-card border rounded-lg shadow-sm">
            {/* Single Scrollable Container for Syncing Header & Body */}
            <div className="flex-1 overflow-auto custom-scrollbar relative">
                <div className="grid" style={{ ...gridStyle, minWidth }}>

                    {/* Sticky Header Row — each cell is individually sticky (contents removes wrapper from layout) */}
                    {/* Time Header Corner */}
                    <div className="sticky top-0 left-0 z-30 bg-muted/30 p-3 border-b border-r font-medium text-xs text-center text-muted-foreground flex items-center justify-center h-[60px]">
                        Time
                    </div>
                    ...
                    {visibleDates.map(date => (
                        <div key={`header-${date.toISOString()}`} className={cn(
                            "sticky top-0 z-20 h-[60px] p-2 border-b border-r font-semibold text-sm text-center flex flex-col items-center justify-center bg-muted/30",
                    ...
                    {/* Grid Body — each hour row uses contents to stay in grid flow */}
                    {HOURS.map(hour => (
                        <div key={`row-${hour}`} className="contents">
                            {/* Time Label */}
                            <div className="sticky left-0 z-10 bg-card border-b border-r flex items-center justify-center text-xs text-muted-foreground font-medium p-2 min-h-[100px]">
