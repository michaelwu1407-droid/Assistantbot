...
- **Deployment staging path**: The GitHub Actions worker deploy uploads a tarball to `/tmp/earlymark-agent-deploy-${GITHUB_SHA}.tgz`, installs it into a staged `/opt/earlymark-agent.release-${GITHUB_SHA}` directory, and only then swaps it into `/opt/earlymark-agent`.
...
- **Automation model**: GitHub Actions deploys `livekit-agent/**` by packaging that folder plus the canonical `earlymark-*.service` files, copying them to the OCI host, validating and installing the staged worker release first, then swapping it into `/opt/earlymark-agent`, reloading `systemd`, and restarting `earlymark-sales-agent` plus `earlymark-customer-agent`. If worker restart or heartbeat convergence fails, the workflow rolls back to `/opt/earlymark-agent.prev`.
