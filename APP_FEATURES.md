...
- Browser push notifications (requires VAPID keys; opt-in via service worker subscription)
- Send test notification button

**WhatsApp Notifications (per-user, opt-in):**
- Per-type toggles backed by the `NotificationChannelPref` table (`userId`, `notificationType`, `channel`)
- Current catalog (first cut): `new_lead`, `ai_call_completed`, `booking_confirmed`; planned: `stale_deal`, `morning_briefing`, `payment_received`
- Messages sent via shared Earlymark Twilio number (`TWILIO_WHATSAPP_NUMBER`) to the user's `User.phone`
- Every outbound body ends with `Reply: ACCEPT N-<code> / REJECT N-<code>` using the last 10 chars of the notification id
- **Two-way replies:** inbound WhatsApp webhook pre-parses `/^(ACCEPT|REJECT|CONFIRM|SNOOZE|OK)\s+N-([a-z0-9]{6,})/i` and routes through the shared `executeNotificationAction` (same code path as the in-app action buttons). Ownership enforced via Prisma `id: { endsWith: code }` + `userId`. Misses fall through to the Tracey AI agent.
- Gated by `WHATSAPP_NOTIFICATIONS_ENABLED` env flag (defaults off until the Earlymark sender clears Twilio error 63007 — Meta registration)
- Observability: `webhookEvent` rows for `whatsapp.outbound.notification` (status: success/skipped_no_phone/skipped_disabled/error), `whatsapp.reply.action_code`, `whatsapp.reply.ai_fallback`, `whatsapp.action.executed`
