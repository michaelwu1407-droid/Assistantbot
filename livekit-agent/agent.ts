...
async function buildVoiceAgentRuntimeSummary() {
  return {
    ...
    requireGuardForOpeners: process.env.VOICE_REQUIRE_GUARD_FOR_OPENERS ?? "true",
  ...
  entry: async (ctx) => {
    ...
    console.log(`${logPrefix} Call started ${JSON.stringify({
      ...
      voiceLatency: {
        ...
        requireGuardForOpeners: voiceLatencyConfig.requireGuardForOpeners,
    ...
    session.on(voice.AgentSessionEventTypes.UserInputTranscribed, async (ev) => {
      ...
      if (
        ...
        openerEntry = resolveOpenerEntry({
          prediction: finalPrediction,
          guardDecision,
          requireGuardDecision: voiceLatencyConfig.requireGuardForOpeners,
