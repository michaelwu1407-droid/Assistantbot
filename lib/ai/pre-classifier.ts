    ...
    case "invoice":
      return [
        ...
        /\b(quote|estimate|proposal)\b/i.test(text)
          ? "QUOTE = DRAFT INVOICE: When the user asks to create a quote or estimate, always try createDraftInvoice first (not createJobNatural), even if they named the customer rather than the exact job title. If a dollar amount is given, update the actual invoice total via updateInvoiceFields (with total: amount) after creation — this correctly updates the invoice document. Also update the deal's tracked amount via updateInvoiceAmount. After creating the draft and setting the amount, move the deal to 'Quote Sent' if it is still in 'New Request'. Only ask whether to create a new job if createDraftInvoice cannot resolve any existing job."
          : null,
