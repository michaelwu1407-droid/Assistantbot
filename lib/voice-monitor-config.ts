const DEFAULT_VOICE_MONITOR_STALE_AFTER_MINUTES = 30;
const DEFAULT_VOICE_SYNTHETIC_PROBE_STALE_AFTER_MINUTES = 240;
const DEFAULT_VOICE_SYNTHETIC_PROBE_REFRESH_AFTER_MINUTES = 180;
...
export function getVoiceSyntheticProbeStaleAfterMinutes(env: NodeJS.ProcessEnv = process.env) {
  return parsePositiveInt(
    env.VOICE_SYNTHETIC_PROBE_STALE_AFTER_MINUTES,
    DEFAULT_VOICE_SYNTHETIC_PROBE_STALE_AFTER_MINUTES,
  );
}

export function getVoiceSyntheticProbeStaleAfterMs(env: NodeJS.ProcessEnv = process.env) {
  return getVoiceSyntheticProbeStaleAfterMinutes(env) * 60_000;
}

export function getVoiceSyntheticProbeRefreshAfterMinutes(env: NodeJS.ProcessEnv = process.env) {
  return parsePositiveInt(
    env.VOICE_SYNTHETIC_PROBE_REFRESH_AFTER_MINUTES,
    DEFAULT_VOICE_SYNTHETIC_PROBE_REFRESH_AFTER_MINUTES,
  );
}

export function getVoiceSyntheticProbeRefreshAfterMs(env: NodeJS.ProcessEnv = process.env) {
  return getVoiceSyntheticProbeRefreshAfterMinutes(env) * 60_000;
}
