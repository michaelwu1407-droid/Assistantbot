...
- **AI Agent Tool-Use Refactor**: Refactored AI agent from context-stuffing (injecting full schedule + message history into system prompt) to a tool-use architecture with just-in-time retrieval. Removed `getNext7DaysSchedule` and `getLast50Messages` injection. Added 6 agent tools: `getSchedule`, `searchJobHistory`, `getFinancialReport`, `getClientContext`, `getTodaySummary`, `getAvailability`. Updated system prompt to instruct model to call tools for data retrieval.
- **Vercel Deploy Fix**: Regenerated `pnpm-lock.yaml` to sync with `package.json`, fixing `ERR_PNPM_OUTDATED_LOCKFILE` build failure.
- **Files Touched**: `actions/agent-tools.ts` (new), `actions/automated-message-actions.ts` (new), `actions/chat-actions.ts`, `actions/deal-actions.ts`, `actions/invite-actions.ts` (new), `actions/notification-actions.ts` (new), `app/api/chat/route.ts`, `app/api/cron/booking-reminders/route.ts` (new), `app/api/extension/import/route.ts`, `app/api/webhooks/email/route.ts`, `app/api/webhooks/stripe/route.ts`, `app/dashboard/settings/notifications/page.tsx`, `app/dashboard/team/page.tsx`, `app/invite/join/page.tsx` (new), `components/chatbot/chat-interface.tsx`, `components/crm/contact-header.tsx`, `components/crm/contact-profile.tsx`, `components/crm/inbox-view.tsx`, `components/layout/Shell.tsx`, `components/layout/mobile-sidebar.tsx`, `components/map/google-map-view.tsx`, `components/modals/new-deal-modal.tsx`, `components/ui/address-autocomplete.tsx`, `prisma/schema.prisma`, `pnpm-lock.yaml`.
...
*   **Cleanup**: Deleted stale `app/dashboard` directory, removed unused `client-page.tsx`, consolidated `job-actions.ts`.
*   **Features**:
    *   Wired `MaterialPicker` to `JobBottomSheet` and `EstimatorForm`.
    *   Completed `EstimatorForm` (Quick Quote) implementation.
    *   Fully migrated Tradie Dashboard to `TradieDashboardClient`.
*   **Files modified**: `components/tradie/estimator-form.tsx`, `components/tradie/material-picker.tsx`, `components/tradie/job-bottom-sheet.tsx`, `components/tradie/tradie-dashboard-client.tsx`, `app/(dashboard)/tradie/page.tsx`, `task.md`, `walkthrough.md`.
*   **Status**: Build GREEN (0 errors). All Critical Fixes & Hygiene items complete.
