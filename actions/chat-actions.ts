...
const WORK_CATEGORIES: Record<string, string[]> = {
  ...
  "Roofing": ["roof", "gutter", "flashing", "tile", "leak", "sagging", "colorbond"],
...
function categoriseWork(desc: string): string {
...
function resolveSchedule(raw: string): { display: string; iso: string } {
  ...
  // Resolve the day to an actual date
  const now = new Date();
...
function enrichAddress(raw: string): string {
